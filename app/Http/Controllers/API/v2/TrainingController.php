...
class TrainingController extends Controller
{
    ...
    function postNewRecord(
        ...
        if (!preg_match("/^([A-Z0-9]{2,3})(_([A-Z0-9]{1,3}))?_(DEL|GND|TWR|APP|DEP|CTR|FSS)$/", $position)) {
    ...
    function postOTSEval(
        ...
        if (!$position || !preg_match('/^([A-Z0-9]{2,3})(_([A-Z]{1,3}))?_(GND|TWR|APP|CTR)$/', $position)) {
            return response()->api(generate_error("Invalid position."), 400);
        }
        try {
    ...
    function editRecord(
        ...
        if (!preg_match("/^([A-Z0-9]{2,3})(_([A-Z]{1,3}))?_(DEL|GND|TWR|APP|DEP|CTR|FSS)$/", $position)) {
            return response()->api(generate_error("Invalid position."), 400);
        }
        if (!in_array(intval($location), [0, 1, 2])) {
            return response()->api(generate_error("Invalid session location. Must be 0, 1, or 2."), 400);
    ...
    function canView(
        ...
        $isVATUSAStaff = Auth::user() ? RoleHelper::isSeniorStaff(Auth::user(), null, true) : false;
