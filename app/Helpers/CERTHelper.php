    ...
    public static function changeRating($cid, $newRating, $addToDatabase = false) {
        if (env('APP_ENV', 'prod') != "prod") {
            if ($addToDatabase) {
                ...
                log_action($cid, "Rating set to " . RatingHelper::intToShort($newRating) . " by " . \Auth::user()->fullname() . " (" . \Auth::user()->cid . ")");
            }
            # Don't do anything
            return 1;
        }

        if (!is_numeric($newRating)) { $newRating = RatingHelper::shortToInt($newRating); }
